...
resource "coxedge_network_policy_rule" "rke2-mgmt-nodes" {
  organization_id  = var.coxedge_organization_id
  environment_name = var.coxedge_environment_name
  network_policy {
    workload_id = coxedge_workload.rke2-mgmt-nodes.id
...
resource "coxedge_network_policy_rule" "rke2-work-nodes" {
  organization_id  = var.coxedge_organization_id
  environment_name = var.coxedge_environment_name
  network_policy {
    workload_id = coxedge_workload.rke2-work-nodes.id
    description = "Allow SSH from Jump"
    protocol    = "TCP"
    type        = "INBOUND"
    action      = "ALLOW"
...
# Mgmt Nodes
resource "coxedge_workload" "rke2-mgmt-nodes" {
  name             = "rke2-ran-mgmt"
  organization_id  = var.coxedge_organization_id
  environment_name = var.coxedge_environment_name
  type             = "VM"
  ...
  user_data = templatefile("cloud-init-mgmt.tpl", {
...
# Worker Nodes
resource "coxedge_workload" "rke2-work-nodes" {
  name             = "rke2-ran-work"
  organization_id  = var.coxedge_organization_id
  environment_name = var.coxedge_environment_name
  type             = "VM"
  ...
  user_data = templatefile("cloud-init-work.tpl", {
    cluster_token = rancher2_cluster_v2.rke2-cluster.cluster_registration_token.0.token
    coxedge_organization_id  = var.coxedge_organization_id
    coxedge_environment_name = var.coxedge_environment_name
    rancher_url = var.rancher_url
